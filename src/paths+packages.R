...
apportion.inv.dist <- function(reach, dist, w, col.names=c("reach", "frac.depletion")){
  ...
  #
  # inputs are two vectors of equal length:
  #   reach = one with the reach number (grouping variable)
  #   dist = distance to the well (units don't matter as long as consistent)
  #   w = weighting exponent (1 for linear, 2 for squared)
  #   col.names = optional character vector for output column names
  
  # for each reach, find minimum distance
  df.temp <- dplyr::summarize(group_by(data.frame(reach = reach,
                                                  dist = dist),
                                       reach),
                              dist.min = min(dist))
  
  # calcualte depletion fraction
  df.temp$frac.depletion <- (1/df.temp$dist.min^w)/sum((1/df.temp$dist.min^w))
  
  # get rid of unnecessary column
  df.temp$dist.min <- NULL
...
apportion.web.dist <- function(reach, dist, w, col.names=c("reach", "frac.depletion")){
  # function to calculate web inverse distance weighting
  #
  # inputs are two vectors of equal length:
  #   reach = one with the reach number (grouping variable)
  #   dist = distance to the well (units don't matter as long as consistent)
  ...
  # calculate depletion based on distance for each point
  frac.depletion <- (1/dist^w)/sum((1/dist^w))
  
  # for each reach, find minimum distance
  df.temp <- dplyr::summarize(group_by(data.frame(reach = reach,
                                                  frac.depletion = frac.depletion),
                                       reach),
                              frac.depletion.sum = sum(frac.depletion))
...
apportion.tpoly <- function(reach,        # vector of reaches
                            dist,         # vector of distance
                            lon,          # vector of longitudes for stream points
                            lat,          # vector of latitudes for stream points
                            wel.lon,      # longitude of well
                            wel.lat,      # latitude of well
                            wel.num,      # well number
                            local.area.m, # local area [m]
                            coord.ref=CRS(crs.MODFLOW), # coordinate reference system
                            col.names=c("reach", "frac.depletion")){  # column names
  # function to calculate Thiessen Polygon weighting
  
  # extent of bounding box which is location of well +/- local area distance in all directions
  wel.extent <- extent(c(wel.lon-local.area.m,
                         wel.lon+local.area.m,
                         wel.lat-local.area.m,
                         wel.lat+local.area.m))
  
  # get closest point on each stream reach to the well
  df.wel.dist <- data.frame(ReachNum = reach,
                            distToWell = dist,
                            lon = lon,
                            lat = lat)
  df.wel.dist.closest <- summarize(group_by(df.wel.dist, ReachNum),
  ...
  spdf.str.closest.to.wel <- SpatialPointsDataFrame(coords = df.wel.dist.closest[,c("lon", "lat")], 
                                                    ...
                                                    proj4string = coord.ref)
  
  # make polygons for all stream points with no wells
  str.vor <- suppressWarnings(voronoi(crop(spdf.str.closest.to.wel, wel.extent), ext=wel.extent))
  
  # make polygon for all stream points AND well
  spdf.wel.str.lonlat <- 
    data.frame(lon = wel.lon, lat=wel.lat,
               ReachNum=-9999, dist.closest=0) %>% 
    SpatialPointsDataFrame(coords = data.frame(lon = wel.lon, lat=wel.lat),
                           data = ., 
                           proj4string = coord.ref) %>% 
    rbind(., spdf.str.closest.to.wel)
  wel.str.vor <- suppressWarnings(voronoi(crop(spdf.wel.str.lonlat, wel.extent), ext=wel.extent))
  wel.str.vor@data$WellNum <- wel.num
  
  # for the polygon containing the well (ReachNum=-9999), figure out what % is contained
  # in the polygons corresponding to each stream reach without the 
  shp.overlap <- suppressWarnings(intersect(subset(wel.str.vor, ReachNum==-9999)[,c("WellNum")], str.vor))
  shp.overlap@data$area.m2 <- area(shp.overlap)
  
  df.temp <- data.frame(ReachNum = shp.overlap@data$ReachNum,
                        frac.depletion = shp.overlap@data$area.m2/sum(shp.overlap@data$area.m2))
