...
df.out %>% 
  dplyr::select(SegNum, WellNum, Time, analytical, distToWell.min.m,
                f.InvDist, f.InvDistSq, f.Web, f.WebSq, f.TPoly, 
                Qf.InvDist, Qf.InvDistSq, Qf.Web, Qf.WebSq, Qf.TPoly) %>% 
  write.csv(., file.path("results", "Depletion_Apportionment+Analytical_AllMethods+Wells+Reaches.csv"), row.names=F)
