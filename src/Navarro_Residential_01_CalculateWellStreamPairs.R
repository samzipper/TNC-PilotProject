## Navarro_Residential_01_CalculateWellStreamPairs+Depletion.R
...
# structure locations shapefile from TNC
sf.houses <- 
...
sf.houses$elev_m <- raster::extract(r.dem.30m, sf.houses)  # elevation of that grid cell [m]
sf.houses$dtb_m <- raster::extract(r.dtb, sf.houses)       # depth to bedrock 
sf.houses$wte_m <- raster::extract(r.wte, sf.houses)       # depth to bedrock 
sf.houses[,c("lon", "lat")] <- sf::st_coordinates(sf.houses)

# for each house location, estimate screen top and bottom based on closest point
dist_houses_wcr <- 
  st_distance(x=sf.wcr, y=sf.houses)
df.houses.wcr <- data.frame(
  WCR.Number = rep(sf.wcr$WCR.Number, times = dim(dist_houses_wcr)[2]),
  Well.Use = rep(sf.wcr$Planned.Us, times = dim(dist_houses_wcr)[2]),
  screenTopDepth_m = rep(sf.wcr$screenTopDepth_m, times = dim(dist_houses_wcr)[2]),
  screenBotDepth_m = rep(sf.wcr$screenBotDepth_m, times = dim(dist_houses_wcr)[2]),
  HouseNum = rep(sf.houses$HouseNum, each = dim(dist_houses_wcr)[1]),
  dist_WCRtoGrow_m = as.numeric(dist_houses_wcr)
)

df.houses.wcr.closest <- 
  df.houses.wcr %>% 
  group_by(HouseNum) %>% 
...
sf.houses <- left_join(sf.houses, df.houses.wcr.closest, by="HouseNum")
...
dist_all <- 
  st_distance(x=sf.streams, y=sf.houses)
...
df.all <- 
  data.frame(
    ...
    HouseNum = rep(sf.houses$HouseNum, each = dim(dist_all)[1]),
    well_elev_m = rep(sf.houses$elev_m, times = dim(dist_all)[1]),
    well_screenTopDepth_m = rep(sf.houses$screenTopDepth_m, times = dim(dist_all)[1]),
    well_screenBotDepth_m = rep(sf.houses$screenBotDepth_m, times = dim(dist_all)[1]),
    well_wte_m = rep(sf.houses$wte_m, times = dim(dist_all)[1]),
    well_dtb_m = rep(sf.houses$dtb_m, times = dim(dist_all)[1]),
  ...
  group_by(HouseNum) %>% 
  top_n(-max.well.segs, dist_wellToStream_m)  # - value to select smallest distances for each HouseNum
...
for (i in 1:dim(df.all)[1]){
  ...
  sf.connector <- st_nearest_points(subset(sf.houses, HouseNum==df.all$HouseNum[i]), 
...
df.all %>% 
  dfDigits(x=., digits=3) %>% 
  write.csv(file.path(dir.TNC, "DerivedData", "Navarro_Residential_01_CalculateWellStreamPairs.csv"),
            row.names=F)
...
ggplot() +
  ...
  geom_sf(data=sf.houses, shape=21)
