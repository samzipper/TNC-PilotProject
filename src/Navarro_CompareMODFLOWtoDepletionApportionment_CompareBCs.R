...
df.fit.well <-
  ...
  subset(Time %in% times_plot &
           pump %in% pump_plot) %>% 
  group_by(stream_BC, pump, WellNum) %>% 
...
df.fit.dist <- 
  ...
  subset(Time %in% times_plot &
           pump %in% pump_plot) %>% 
  group_by(stream_BC, pump, dist.cut) %>% 
  summarize(n.reach = sum(is.finite(depletion.prc)),
            cor = cor(depletion.prc, depletion.prc.modflow, method="pearson"),
            bias = pbias(depletion.prc, depletion.prc.modflow),
            R2 = R2(depletion.prc, depletion.prc.modflow),
...
df.fit.dist.vert <- 
  ...
  subset(Time %in% times_plot &
           pump %in% pump_plot) %>% 
  group_by(stream_BC, pump, dist.vert.cut) %>% 
  summarize(n.reach = sum(is.finite(depletion.prc)),
            cor = cor(depletion.prc, depletion.prc.modflow, method="pearson"),
            bias = pbias(depletion.prc, depletion.prc.modflow),
            R2 = R2(depletion.prc, depletion.prc.modflow),
...
df.fit.dist.vert.wellscreen <- 
  ...
  subset(Time %in% times_plot &
           pump %in% pump_plot) %>% 
  group_by(stream_BC, pump, dist.vert.wellscreen.cut) %>% 
  summarize(n.reach = sum(is.finite(depletion.prc)),
            cor = cor(depletion.prc, depletion.prc.modflow, method="pearson"),
            bias = pbias(depletion.prc, depletion.prc.modflow),
            R2 = R2(depletion.prc, depletion.prc.modflow),
...
df.fit.length <-
  ...
  subset(Time %in% times_plot &
           pump %in% pump_plot) %>% 
  group_by(stream_BC, pump, length.cut) %>% 
  summarize(n.reach = sum(is.finite(depletion.prc)),
            cor = cor(depletion.prc, depletion.prc.modflow, method="pearson"),
            bias = pbias(depletion.prc, depletion.prc.modflow),
            R2 = R2(depletion.prc, depletion.prc.modflow),
...
## scatterplot of fit in SFR vs RIV
df.fit.well %>%
  dplyr::select(stream_BC, WellNum, depletion.prc.modflow.mean) %>% 
  reshape2::dcast(WellNum ~ stream_BC) %>% 
  ggplot(aes(x=RIV, y=SFR)) +
  geom_point() +
  geom_abline(intercept=0, slope=1) 

## fit elevation quartile
df.fit.dist %>% 
  transform(MAE.norm = MAE.overall/(depletion.prc.modflow.max-depletion.prc.modflow.min)) %>% 
  dplyr::select(stream_BC, dist.cut,dist.m.median, MAE.norm) %>% 
  reshape2::dcast(dist.cut ~ stream_BC) %>% 
  ggplot(aes(x=RIV, y=SFR)) +
  geom_point() +
  geom_abline(intercept=0, slope=1) 
