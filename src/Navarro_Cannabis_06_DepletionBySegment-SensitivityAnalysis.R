## Navarro_Cannabis_06_DepletionBySegment-SensitivityAnalysis.R
#' This script is intended to calculate depletion for stream segments as
#' a sensitivity analysis relative to Navarro_Cannabis_05_DepletionBySegment.R
#' 
#' Specifically, it will increase and decrease pumping by 50% relative to the
...
# pumping factor (to multiply mean monthly pumping rate by)
...
df.Qs %>% 
  ...
  dplyr::select(SegNum, WellNum, time_days, Qs, frac_depletion, depletion_m3d) %>% 
  dfDigits(x=., digits=7) %>% 
