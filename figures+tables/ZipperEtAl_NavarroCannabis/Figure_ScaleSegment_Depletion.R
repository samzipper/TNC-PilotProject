...
############# TEMPORARY
names(df.depletion)[names(df.depletion)=="Qf"] <- "depletion_m3d"
...
df.interp.1 <- data.frame(Z = subset(sf.value, IP_class=="High" & time_d == times_plot[1])$depletion_m3d.HighValue,
                          X = st_coordinates(subset(sf.value, IP_class=="High" & time_d == times_plot[1]))[,"X"],
                          Y = st_coordinates(subset(sf.value, IP_class=="High" & time_d == times_plot[1]))[,"Y"])
...
df.Qf.t.1 <- 
  ...
  transform(time_d = times_plot[1])

df.interp.2 <- data.frame(Z = subset(sf.value, IP_class=="High" & time_d == times_plot[2])$depletion_m3d.HighValue,
                          X = st_coordinates(subset(sf.value, IP_class=="High" & time_d == times_plot[2]))[,"X"],
                          Y = st_coordinates(subset(sf.value, IP_class=="High" & time_d == times_plot[2]))[,"Y"])
...
df.Qf.t.2 <- 
  ...
  as.data.frame() %>% 
  set_colnames(c("lon", "lat", "depletion_m3d.HighValue")) %>% 
  transform(time_d = times_plot[2])

df.interp.3 <- data.frame(Z = subset(sf.value, IP_class=="High" & time_d == times_plot[3])$depletion_m3d.HighValue,
                          X = st_coordinates(subset(sf.value, IP_class=="High" & time_d == times_plot[3]))[,"X"],
                          Y = st_coordinates(subset(sf.value, IP_class=="High" & time_d == times_plot[3]))[,"Y"])
...
## build labeller
time_labels <- 
  setNames(paste0("Year ", times_plot/365),
           times_plot)

time_labels_ac <- 
  setNames(paste0(c("(a) ", "(b) ", "(c) "), "Year ", times_plot/365),
           times_plot)
...
df.buff.mean <-
  df.buff %>% 
  group_by(time_d, buff_range_center) %>% 
...
p.dist <- 
  ...
  scale_x_continuous(name=paste0("Buffer Distance [center of ", buff_interval, " m bin]")) +
...
subset(df.buff, time_d == max(df.buff$time_d) & buff_dist==3800 & depletion_m3d_HighValue > 0.1)
