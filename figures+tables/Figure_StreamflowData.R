...
df.mo.trend <- data.frame(month=seq(1,12),
                          ...
                          p = NaN)
for (mo in seq(1,12)){
  df.mo.trend$R2[mo] <- summary(lm(baseflow.mm_d.mean ~ year, subset(df.yr.mo, month==mo)))$r.squared
  df.mo.trend$p[mo] <- lmp(lm(baseflow.mm_d.mean ~ year, subset(df.yr.mo, month==mo)))
}
