...
p.match.fit <- 
  ...
  geom_line(aes(x=Time/365, y=MAE.match/(depletion.prc.modflow.max-depletion.prc.modflow.min), color=apportionment)) +
...
p.sum <- 
  ...
  geom_rect(data=df.NoPump.times, 
            ...
            fill=col.gray, alpha=0.25) +
  geom_hline(yintercept=0, color=col.gray) +
  geom_line(aes(x=Time/365, y=MAE.sum/(Qf.total.MODFLOW.max-Qf.total.MODFLOW.min), color=apportionment)) +
